// CollectionPageRootStraps.jsx — My Collection root + Strap Drawer entry points.
// Same as CollectionPageRoot, with: (1) a compact Strap Drawer link in the header
// meta row, and (2) the editorial Strap Drawer band below the watchbox.

const IconPlusS = ({ size = 13 }) =>
...
const CollectionPageRoot = ({ setPage }) => {
  ...
  // WatchData paths are authored relative to collection_redesign/ (../assets/…);
  // this page lives at the project root, so normalize to assets/…
  const [watches, setWatches] = React.useState(
    () => WATCHES.map(w => ({ ...w, imageUrl: w.imageUrl ? w.imageUrl.replace(/^\.\.\//, '') : w.imageUrl }))
  );
  ...
  return React.createElement('div', { style: { minHeight: '100vh', background: '#FAF8F4' } },
    ...
    React.createElement('div', { style: { maxWidth: 1280, margin: '0 auto', padding: '40px 56px 0' } },
      ...
      // Meta row: Add Watch + quiet stats + Strap Drawer link
      React.createElement('div', {
        ...
        React.createElement('button', {
          ...
          React.createElement(IconPlusS, { size: 13 }),
          'Add Watch'
        ),
        ...
        // Compact Strap Drawer entry (sits right after Stats)
        React.createElement('span', { style: { width: 1, height: 26, background: '#EAE5DC' } }),
        React.createElement(StrapHeaderLink),
    ...
    // ============ STRAP DRAWER BAND ============
    React.createElement(StrapDrawerBand),
...
Object.assign(window, { CollectionPageRoot });
