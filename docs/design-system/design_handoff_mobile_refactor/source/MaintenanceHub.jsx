...
const SummaryStrip = ({ collection }) => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 0, background: C.card,
      ...
      {stats.map((s, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            borderLeft: col ? `1px solid ${C.border}` : 'none',
            borderTop: row ? `1px solid ${C.border}` : 'none',
          }}>
            <Meta style={{ display: 'block', marginBottom: 8 }}>{s.label}</Meta>
            <div style={{ fontFamily: C.serif, fontSize: isMobile ? 28 : 34, fontWeight: 400, color: s.accent || C.ink, lineHeight: 0.95, marginBottom: 6 }}>{s.value}</div>
            <span style={{ fontFamily: C.sans, fontSize: 11, color: C.muted, display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.meta}</span>
          </div>
        );
      })}
    </div>
...
const MaintenanceHub = () => {
  ...
  const [vw, setVw] = React.useState(() => (typeof window !== 'undefined' ? window.innerWidth : 1320));
  React.useEffect(() => {
  ...
  const onSaveService = (watch, record, documents = []) => {
    setCollection(cs => cs.map(w => w.id === watch.id
      ...
      : w));
    setLogFor(null);
    const dn = documents && documents.length ? ` · ${documents.length} doc${documents.length > 1 ? 's' : ''} filed` : '';
    flash(`${serviceType(record.type).label} logged for ${watch.brand} ${watch.model}${dn}`);
  };
  ...
  return (
    <ViewportCtx.Provider value={{ isMobile, vw }}>
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 60, background: 'rgba(250,248,244,0.88)', backdropFilter: 'blur(10px)', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: `0 ${pad}px`, height: isMobile ? 54 : 62, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontFamily: C.sans, fontSize: 13, fontWeight: 600, letterSpacing: '0.02em', color: C.ink, whiteSpace: 'nowrap' }}>{isMobile ? 'Service Room' : 'The Service Room'}</span>
          </div>
          {isMobile ? (
            <button type="button" aria-label="Menu" style={{ width: 36, height: 36, borderRadius: 8, border: `1px solid ${C.border}`, background: C.card, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
              <Icon name="list" size={16} color={C.ink} />
            </button>
          ) : (
      ...
      {/* header */}
      <header style={{ maxWidth: 1320, margin: '0 auto', padding: `${isMobile ? 26 : 40}px ${pad}px 0` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 24, marginBottom: isMobile ? 20 : 26, flexWrap: 'wrap' }}>
          <div>
            <Meta style={{ color: C.gold, display: 'block', marginBottom: 10 }}>Maintenance &amp; provenance</Meta>
            <h1 style={{ fontFamily: C.serif, fontSize: isMobile ? 34 : 52, fontWeight: 300, color: C.ink, lineHeight: 0.98, letterSpacing: '-0.01em', margin: 0 }}>
              The Service Room
            </h1>
            <p style={{ fontFamily: C.sans, fontSize: isMobile ? 13 : 14, color: C.muted, lineHeight: 1.6, margin: '12px 0 0', maxWidth: 460 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, margin: isMobile ? '20px 0 4px' : '28px 0 4px', flexWrap: 'wrap' }}>
          <LayoutSwitch value={layout} onChange={setLayout} />
          {isMobile ? (
            <button type="button" onClick={onExportAll} style={{ ...btnSecondary, padding: '9px 14px' }}>
              <Icon name="download" size={14} color={C.ink} />Export
            </button>
          ) : (
      ...
      {/* main */}
      <main style={{ maxWidth: 1320, margin: '0 auto', padding: `24px ${pad}px 40px`, display: 'flex', flexDirection: 'column', gap: isMobile ? 30 : 40 }}>
        <Layout collection={collection} onPick={onPick} onLog={onLog} activeId={selectedId} />
        <div style={{ height: 1, background: C.border }} />
        <PartnerBand />
      </main>

      <footer style={{ maxWidth: 1320, margin: '0 auto', padding: `24px ${pad}px 48px`, borderTop: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
