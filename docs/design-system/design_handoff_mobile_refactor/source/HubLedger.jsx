    ...
    return (
      <div>
        <SectionHead eyebrow="The file cabinet" title="Every piece, on the record"
          hint="Tap a card to open the dossier" />

        {/* sort control */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          <Meta>Sort</Meta>
          <select value={sort.key} onChange={e => setSort({ key: e.target.value, dir: 1 })} style={{
            fontFamily: C.sans, fontSize: 12.5, color: C.ink, background: C.card, border: `1px solid ${C.borderLight}`,
            borderRadius: 8, padding: '7px 11px', flex: 1, outline: 'none',
          }}>
          ...
          <button type="button" onClick={() => setSort(s => ({ ...s, dir: -s.dir }))} title="Reverse order" style={{
            ...iconBtn, width: 36, height: 36,
          }}>
            <span style={{ fontSize: 11, color: C.muted, transform: sort.dir < 0 ? 'rotate(180deg)' : 'none', display: 'inline-block' }}>▾</span>
          </button>
        </div>
          ...
          {rows.map(w => {
            ...
            return (
              <div key={w.id} onClick={() => onPick(w)} style={{
                background: C.card, border: `1px solid ${C.border}`, borderLeft: `3px solid ${st.dot}`,
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 12px', margin: '13px 0', paddingTop: 13, borderTop: `1px solid ${C.border}` }}>
                  <LedgerCell label="Last serviced" value={la ? fmtMonthYear(la.date) : '—'} sub={la ? serviceType(la.type).label : null} />
                  <LedgerCell label="Next due" value={fmtDate(st.due, { year: 'numeric', month: 'short' })} accent={st.fg} />
                  <LedgerCell label="Lifetime upkeep" value={fmt(lifetimeCost(w))} />
                  <LedgerCell label="Warranty" value={ws ? (ws.key === 'expired' ? 'Expired' : fmtMonthYear(ws.date)) : '—'} accent={ws ? ws.fg : C.muted} />
                </div>

                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: C.muted }}>
                    <Icon name="doc" size={14} color={C.muted} />
                    <span style={{ fontFamily: C.sans, fontSize: 12.5, color: C.ink }}>{(w.documents || []).length} on file</span>
                    {!w.hasPapers && <span style={{ fontFamily: C.sans, fontSize: 9.5, color: STATUS.due.fg, background: STATUS.due.bg, padding: '1px 7px', borderRadius: 10 }}>no papers</span>}
                  </span>
                  <button type="button" onClick={e => { e.stopPropagation(); onLog(w); }} style={{ ...btnSecondary, padding: '7px 13px' }}>
                    <Icon name="plus" size={13} color={C.ink} />Log
                  </button>
                </div>
              </div>
            );
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14,
          padding: '14px 16px', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 11 }}>
          <span style={{ fontFamily: C.sans, fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: C.ink }}>{collection.length} pieces · {totalDocs} docs</span>
          <span style={{ fontFamily: C.serif, fontSize: 20, fontWeight: 500, color: C.gold }}>{fmt(totalCost)}</span>
        </div>
      </div>
    );
  }
...
const LedgerCell = ({ label, value, sub, accent }) => (
  <div>
    <Meta style={{ display: 'block', marginBottom: 3, fontSize: 9 }}>{label}</Meta>
    <span style={{ fontFamily: C.sans, fontSize: 13, fontWeight: 600, color: accent || C.ink }}>{value}</span>
    {sub && <div style={{ fontFamily: C.sans, fontSize: 10.5, color: C.muted, marginTop: 1 }}>{sub}</div>}
  </div>
);
