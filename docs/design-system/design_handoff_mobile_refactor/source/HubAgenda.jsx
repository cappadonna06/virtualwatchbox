...
const AttentionCard = ({ w, onPick, onLog }) => {
  ...
  const imgSize = isMobile ? 84 : 128;

  return (
    <div style={{
      display: 'grid', gridTemplateColumns: `${imgSize}px 1fr`, gap: isMobile ? 14 : 22, padding: isMobile ? 15 : 20,
      ...
      <button type="button" onClick={() => onPick(w)} style={{
        width: imgSize, height: '100%', minHeight: imgSize, alignSelf: 'stretch', background: 'none', border: 'none',
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
          <div onClick={() => onPick(w)} style={{ cursor: 'pointer', flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <div style={{ fontFamily: C.serif, fontSize: isMobile ? 20 : 23, fontWeight: 400, color: C.ink, lineHeight: 1.06 }}>
              {w.model}
            </div>
          </div>
        ...
        <div style={{
          display: 'flex', alignItems: 'center', gap: 8, marginTop: isMobile ? 10 : 12, marginBottom: isMobile ? 12 : 14,
          fontFamily: C.sans, fontSize: 12.5, fontWeight: 500, color: st.fg,
        }}>
          <Icon name={overdue ? 'clock' : 'calendar'} size={14} color={st.fg} style={{ flexShrink: 0 }} />{need}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: isMobile ? 'stretch' : 'center', justifyContent: 'space-between', gap: isMobile ? 14 : 12, flexDirection: isMobile ? 'column' : 'row', flexWrap: 'wrap' }}>
          <div style={isMobile
            ...
            : { display: 'flex', gap: 22 }}>
            <Stat label={isMobile ? 'Last' : 'Last full'} value={lf ? fmtMonthYear(lf.date) : 'Never'} />
            <Stat label={isMobile ? 'Every' : 'Interval'} value={`${w.intervalYears} yr`} />
            <Stat label={isMobile ? 'Upkeep' : 'Lifetime upkeep'} value={fmt(cost)} />
          </div>
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, width: isMobile ? '100%' : 'auto' }}>
            <button type="button" onClick={() => onLog(w)} style={{ ...btnPrimary, flex: isMobile ? 'none' : 'none', width: isMobile ? '100%' : 'auto' }}>
              <Icon name="plus" size={13} color={C.slot} />Log a service
            </button>
            <a href={bookingUrl(w)} target="_blank" rel="noopener noreferrer" style={{ ...btnSecondary, width: isMobile ? '100%' : 'auto' }}>
  ...
  if (isMobile) {
    return (
      <div style={{
        ...
        padding: '11px 14px', background: C.card, border: `1px solid ${C.border}`, borderRadius: 10,
      }}>
        <button type="button" onClick={() => onPick(w)} style={{
          width: 52, height: 52, background: 'none', border: 'none', cursor: 'pointer', padding: 0,
        ...
        <div onClick={() => onPick(w)} style={{ cursor: 'pointer', minWidth: 0 }}>
          ...
          <span style={{ fontFamily: C.sans, fontSize: 11, color: C.muted }}>Next {fmtDate(st.due, { year: 'numeric', month: 'short' })} · {fmt(lifetimeCost(w))}</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 7 }}>
          <StatusChip status={st} size="sm" />
          <button type="button" onClick={() => onLog(w)} title="Log a service" style={{ ...iconBtn, width: 30, height: 30 }}>
