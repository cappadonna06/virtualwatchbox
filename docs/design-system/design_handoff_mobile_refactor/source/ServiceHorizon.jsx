...
const ServiceHorizon = ({ collection, onPick, activeId }) => {
  ...
  return (
    <div>
      <div style={{ overflowX: isMobile ? 'auto' : 'visible', overflowY: 'hidden', paddingBottom: isMobile ? 6 : 0, WebkitOverflowScrolling: 'touch' }}>
      ...
      <div style={{ display: 'flex', gap: 18, marginTop: 12, flexWrap: 'wrap' }}>
        ...
        {beyondN > 0 && <span style={{ fontFamily: C.sans, fontSize: 11, color: C.muted, marginLeft: isMobile ? 0 : 'auto' }}>{isMobile ? `+${beyondN} past two years` : `${beyondN} resting comfortably past two years`}</span>}
      </div>
    </div>
  );
};
