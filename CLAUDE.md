...
- **Case segmentation pipeline (migration 034, `lib/caseSegmentation.ts`):** a tiered case-only cutout pipeline. `GeometricSilhouetteProvider` (free, deterministic) fits the **case-contour model** to the existing alpha channel — robust circle/ellipse fit of the case body, lug-tip detection via span contraction, mask = body ∪ lug horns ∪ crown hard-stopped at the lug tips, with the between-lugs channel floor **snapped to the actual bezel edge** via gradient-cluster detection (`refineChannelFloor` — the fitted arc alone keeps an end-link sliver over the serrated ring). Low-confidence results escalate to `ClaudeVisionLandmarkProvider` (semantic lug-point detection); `ReplicateSamProvider` remains as a rare heavy fallback. Integrated-bracelet watches (`bracelet_type: 'integrated'`) are skipped outright — that's the Studio's own side-by-side design, not a segmentation failure. The `/admin/image-review` "Case Segmentation" tab (`components/admin/CaseSegmentationReview.tsx`) is built — drag the four lug markers to correct a cutout; `npm run straps:sync-bridge` folds reviewed Supabase state back into the committed `data/case-only-images.json`. Tests: `npm run straps:segment-cases:selftest` (analytic IoU + probes) and `straps:segment-cases:realtest` (committed real photo, visual outputs). See [docs/playbooks/case-segmentation-strategy.md](docs/playbooks/case-segmentation-strategy.md) for the full design.
