...
- **Composite vs side-by-side.** When a watch has an approved case-only render (`getCaseOnly(catalogId)` from `lib/caseOnlyImages.ts`), `StudioComposite` renders the strap **worn on the watch**: two band halves (Delugs-style "as worn" renders) whose spring-bar rows are anchored into the watch's lug channels from `lug_geometry` and width-scaled to fill them snugly, layered behind the case. Otherwise it falls back to a side-by-side layout.
- **Band assets + manifest.** Worn halves live in `public/demo-strap-swap-bands/`; per-half anchors (`pinY`, `bodyLeft/Right` — measured via `scripts/measure-strap-bands.ts`) are committed in `data/strap-band-demo.json`. **Composite mode shows ONLY band-equipped straps** — flat template photos read as "behind the case", so they stay exclusive to side-by-side watches (full template + drawer catalog there).
- **Flat templates are white-bg photos, not transparent.** Side-by-side mode keys them via the inline SVG luminance filter `#studioStrapKey` (near-white → transparent, `feComposite operator="in"` clips back to source alpha so the `object-fit` letterbox doesn't go black). Band halves are genuinely transparent — no filter.
...
- **Layout:** global `NavBar`/`Footer` render normally (no route guards); the page sits inside `.site-main`'s 1280 container. Ghost carousel (stacked band halves, opacity falloff) flanks the composite; arrows are in normal flex flow — never absolutely positioned or given a static transform (Framer's `whileTap` would clobber it and make them jump).
