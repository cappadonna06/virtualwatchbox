...
The premium visual configurator at `/collection/straps/studio` — a dark, full-bleed surface where a watch is shown wearing different straps. See [docs/playbooks/strap-studio-mvp.md](docs/playbooks/strap-studio-mvp.md) and PRD §7.9.

- **Composite vs side-by-side.** When a watch has an approved case-only render (`getCaseOnly(catalogId)` from `lib/caseOnlyImages.ts`), `StudioComposite` layers the strap behind a case-only image positioned from `lug_geometry`. Otherwise it falls back to a side-by-side layout. Both are equally polished.
- **Strap templates are white-bg photos, not transparent.** `StudioComposite` keys them at render via the inline SVG luminance filter `#studioStrapKey` (near-white → transparent, `feComposite operator="in"` clips back to source alpha so the `object-fit` letterbox doesn't go black). Don't replace this with a per-pixel approach.
- **Model layer:** `lib/strapStudio.ts` unifies templates + drawer straps into `StudioStrap`, derives categories from materials actually present (no empty tabs), and computes fit. `components/straps/studio/useStudioController.ts` owns all state (URL-synced `watchId`/`strapId`/`source`, keyboard, aggressive preload via `useStrapPreloader`, incompatible-strap auto-switch).
- **Entry points:** sidebar "Swap Strap" (`WatchSidebar`), strap drawer "Try in Studio" (`StrapSidebar`), watch detail link — all deep-link with the **catalog** id (`watch.watchId`), not the owned id.
- **Brand tokens:** dark-canvas values live under `brand.studio.*` in `lib/brand.ts`. The route hides the global `NavBar`/`Footer` (pathname guards) and breaks out of `.site-main` for full-bleed.
