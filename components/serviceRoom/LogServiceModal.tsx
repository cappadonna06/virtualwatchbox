...
import { formatCost, formatDate, serviceTypeMeta, SERVICE_TYPES, type ServiceWatch } from '@/lib/serviceRoom/derive'
import type { ServiceRecordInput } from '@/app/collection/CollectionSessionProvider'
import type { ServiceType } from '@/types/watch'
...
type Props = {
  ...
  onClose: () => void
  onSave: (sw: ServiceWatch, data: ServiceRecordInput) => void | Promise<void>
}
...
export function LogServiceModal({ sw, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    setDate(today); setType('full'); setProvider(''); setCost(''); setNotes(''); setSaving(false)
