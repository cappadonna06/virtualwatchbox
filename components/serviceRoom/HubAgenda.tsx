...
function AttentionCard({ sw, now, onPick, onLog }: { sw: ServiceWatch; now: Date; onPick: (sw: ServiceWatch) => void; onLog: (sw: ServiceWatch) => void }) {
  ...
  return (
    <div className="service-attention-card" style={{
      ...
      <button type="button" onClick={() => onPick(sw)} aria-label={`Open ${sw.watch.brand} ${sw.watch.model} dossier`} style={{ width: 128, height: 128, background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
          <div onClick={() => onPick(sw)} style={{ cursor: 'pointer', flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <div style={{ fontFamily: serif, fontSize: 23, fontWeight: 400, color: brand.colors.ink, lineHeight: 1.06 }}>{sw.watch.model}</div>
          </div>
          <StatusChip status={st} />
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12, marginBottom: 14, fontFamily: sans, fontSize: 12.5, fontWeight: 500, color: st.fg }}>
          <Icon name={overdue ? 'clock' : 'calendar'} size={14} color={st.fg} />{need}
        </div>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: 22 }}>
            <Stat label="Last full" value={lf ? formatMonthYear(lf.serviceDate) : 'Never'} />
            <Stat label="Interval" value={`${sw.intervalYears} yr`} />
            <Stat label="Lifetime upkeep" value={formatCost(lifetimeCostCents(sw))} />
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={() => onLog(sw)} style={btnPrimary}>
              <Icon name="plus" size={13} color={brand.colors.slot} />Log a service
            </button>
            <a href={bookingUrl(sw.watch.brand)} target="_blank" rel="noopener noreferrer sponsored" style={btnSecondary}>
...
function OnTrackRow({ sw, now, onPick, onLog }: { sw: ServiceWatch; now: Date; onPick: (sw: ServiceWatch) => void; onLog: (sw: ServiceWatch) => void }) {
  const st = serviceStatus(sw, now)
  const lf = lastFullService(sw)
  return (
    <div className="service-ontrack-row" style={{
      ...
      <div className="service-ontrack-hide"><Meta style={{ display: 'block', marginBottom: 2 }}>Next due</Meta>
        <span style={{ fontFamily: sans, fontSize: 12.5, color: brand.colors.ink }}>{formatDate(st.due, { year: 'numeric', month: 'short' })}</span>
      </div>
      <div className="service-ontrack-hide"><Meta style={{ display: 'block', marginBottom: 2 }}>Upkeep</Meta>
...
export function HubAgenda({ watches, now, onPick, onLog, activeId }: LayoutProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 34 }}>
      <section>
        ...
        <ServiceHorizon watches={watches} now={now} onPick={onPick} activeId={activeId} />
      </section>
      ...
      <section>
        ...
        {attention.length ? (
          <div style={{ display: 'grid', gap: 14 }}>
            {attention.map(sw => <AttentionCard key={sw.watch.id} sw={sw} now={now} onPick={onPick} onLog={onLog} />)}
          </div>
        ) : (
      ...
      {onTrack.length > 0 && (
        <section>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {onTrack.map(sw => <OnTrackRow key={sw.watch.id} sw={sw} now={now} onPick={onPick} onLog={onLog} />)}
          </div>
        </section>
      )}
    </div>
