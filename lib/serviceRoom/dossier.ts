// lib/serviceRoom/dossier.ts — build & download a branded, print-ready service
// dossier (whole collection or a single watch). Client-side only.
...
function watchSection(sw: ServiceWatch, now: Date): string {
  const w = sw.watch
  const st = serviceStatus(sw, now)
  const lf = lastFullService(sw)
  const ws = warrantyStatus(sw, now)
  const records = [...sw.records].sort((a, b) => (a.serviceDate < b.serviceDate ? 1 : -1))

  const rows = records.map(r => `
    <tr>
      <td class="dt">${esc(formatDate(r.serviceDate))}</td>
      <td><strong>${esc(serviceTypeMeta(r.serviceType).label)}</strong>${r.notes ? `<div class="note">${esc(r.notes)}</div>` : ''}</td>
      <td>${esc(r.provider ?? 'Unspecified')}</td>
      <td class="num">${r.cost ? esc(formatCost(r.cost)) : '—'}</td>
    </tr>`).join('')

  const docs = sw.documents.map(d => {
    const type = d.photoType as string
    const label = d.caption?.trim() || docTypeMeta(type).label
    return `<li><span class="dtag">${esc(docTypeMeta(type).label)}</span> ${esc(label)} <span class="muted">· ${esc(formatDate(d.takenAt || d.createdAt))}</span></li>`
  }).join('')
  ...
  <section class="watch">
    <div class="whead">
      <div>
        ...
        <h2>${esc(w.model)}</h2>
        <div class="muted">Ref. ${esc(w.reference)} · ${esc(w.caseSizeMm)}mm · ${esc(w.caseMaterial)} · ${esc(w.movement)}</div>
      </div>
      <div class="status ${st.key}">${esc(st.label)} · ${esc(formatDate(st.due, { year: 'numeric', month: 'short' }))}</div>
    </div>

    <div class="summary">
      <div><span class="lbl">Acquired</span>${esc(formatDate(w.purchaseDate))}${w.acquisitionMethod ? ` · ${esc(ACQ_LABEL[w.acquisitionMethod])}` : ''}</div>
      <div><span class="lbl">Last full service</span>${lf ? esc(formatDate(lf.serviceDate)) : 'Never'}</div>
      <div><span class="lbl">Service interval</span>${esc(sw.intervalYears)} years</div>
      <div><span class="lbl">Lifetime upkeep</span>${esc(formatCost(lifetimeCostCents(sw)))}</div>
      <div><span class="lbl">Box &amp; papers</span>${w.hasBox ? 'Box' : 'No box'} · ${w.hasPapers ? 'Papers' : 'No papers'}</div>
      <div><span class="lbl">Warranty</span>${ws ? (ws.key === 'expired' ? 'Expired ' : 'Valid to ') + esc(formatDate(ws.date)) : '—'}</div>
    </div>
...
export function downloadDossier(watches: ServiceWatch[], filename: string, now: Date = new Date()): void {
  if (typeof window === 'undefined' || watches.length === 0) return
  const totalCost = watches.reduce((s, w) => s + lifetimeCostCents(w), 0)
  const isAll = watches.length > 1
  const title = isAll ? 'Collection Service Record' : `${watches[0].watch.brand} ${watches[0].watch.model}`

  const html = `<!DOCTYPE html><html><head><meta charset="utf-8">
<title>${esc(isAll ? 'Collection Service Dossier' : `${watches[0].watch.brand} ${watches[0].watch.model} — Dossier`)}</title>
...
<body><div class="doc">
  ...
  <div class="meta">Generated ${esc(formatDate(now))} · ${watches.length} piece${watches.length === 1 ? '' : 's'} · Lifetime upkeep ${esc(formatCost(totalCost))}</div>
  ${watches.map(w => watchSection(w, now)).join('')}
  ${isAll ? `<div class="totals"><span>Total lifetime maintenance across ${watches.length} pieces</span><span class="big">${esc(formatCost(totalCost))}</span></div>` : ''}
  ...
  a.href = url
  a.download = (filename || 'service-dossier').replace(/[^a-z0-9-]+/gi, '-') + '.html'
